...
def seed_database(seed_path: str = "Data/seed.json"):
    ...
    with get_connection() as conn:
        ...
        for h in data.get("hotels", []):
            cur.execute("""
                INSERT OR IGNORE INTO hotels (id, name, city, price, rating, rooms, available)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (h["id"], h["name"], h["city"], h["price"], h["rating"], h["rooms"], int(h["available"])))
        # users
...
def fetch_hotels(city: str = None, max_price: float = None):
    with get_connection() as conn:
        cur = conn.cursor()
        query = "SELECT id, name, city, price, rating, rooms, available FROM hotels WHERE 1=1"
        params = []
        if city:
            query += " AND city = ?"
            params.append(city)
